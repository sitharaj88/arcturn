...
/**
 * Whatever catches the authorization redirect.
 *
 * The loopback listener is one implementation and the default one, but it is
 * only correct when the browser and this process share a network namespace. An
 * editor attached over SSH, a devcontainer or a Codespace breaks that
 * assumption: the browser opens on the user's machine and `127.0.0.1` there is
 * not `127.0.0.1` here, so the redirect never arrives and the flow times out
 * with nothing to show for it. Clients in that position bring their own
 * redirect — VS Code tunnels one back through `vscode://` — and the flow does
 * not care which it got, because everything that makes the exchange *safe*
 * (PKCE, the `state` echo, the one-shot code) is enforced somewhere else.
 */
export interface McpRedirectListener {
   ...
   *
   * Implementations must verify the `state` they were given before resolving,
   * and reject rather than resolve on denial, timeout or cancellation.
   */
  waitForCallback(): Promise<{ code: string }>;
  /** Stop listening. Must be idempotent, and safe after the promise settles. */
  close(): Promise<void>;
}

/**
 * Binds a {@link McpRedirectListener} for one authorization.
 *
 * Called before the authorization URL is built, because the URL has to carry
 * the redirect URI the listener actually answers on.
 */
export type McpRedirectListenerFactory = (options: {
  /** The `state` the callback must echo, generated by the flow. */
  readonly state: string;
  /** How long to wait before giving up. */
  readonly timeoutMs: number;
  /** Cancels the wait. */
  readonly signal?: AbortSignal;
}) => Promise<McpRedirectListener>;
...
export interface McpOAuthFlowOptions {
  ...
  readonly signal?: AbortSignal;
  /**
   * Where the redirect is caught. Defaults to a loopback listener on
   * `127.0.0.1` with an ephemeral port; see {@link McpRedirectListener} for
   * why a client would supply its own.
   */
  readonly redirect?: McpRedirectListenerFactory;
  /**
   * Called with the authorization URL, before the wait begins.
   *
   * This is how a caller that is not a terminal gets hold of the URL — the
   * default path prints it and opens a browser, which is useless to a client
   * on the other end of a socket. Supplying this does not suppress `stdout` or
   * `open`; pass `stdout: () => undefined` and `open: false` for that.
   */
...
export async function runMcpOAuthFlow(options: McpOAuthFlowOptions): Promise<void> {
  ...
  const timeoutMs = options.timeoutMs ?? DEFAULT_MCP_AUTH_TIMEOUT_MS;
  const bind: McpRedirectListenerFactory =
    options.redirect ??
    ((bindOptions) => oauth.startLoopbackServer({ ...bindOptions, port: 0, path: "/callback" }));
  const listener = await bind({
    state,
    timeoutMs,
...
/** Options for {@link createMcpAuthBroker}. */
export interface McpAuthBrokerFactoryOptions {
  /**
   * The three paths this needs, and only those three.
   *
   * Narrower than {@link ArcturnPaths} on purpose: the serve host's runtime
   * carries `{ home, project }` rather than a resolved layout, and widening
   * that to satisfy a factory would have been the tail wagging the dog. A full
   * `ArcturnPaths` satisfies this structurally.
   */
  readonly paths: Pick<ArcturnPaths, "userMcp" | "projectMcp" | "auth">;
  /** Environment used to expand `${VAR}` in a server URL. Defaults to the process's. */
  readonly env?: Record<string, string | undefined>;
  /** How long a begun authorization survives without its code. */
  readonly timeoutMs?: number;
}

/**
 * Build the broker the serve path's `mcpAuth*` verbs run on.
 *
 * The config is re-read per authorization rather than captured at startup, for
 * the reason `mcpStatus` re-reads the manager: a server added to `.mcp.json`
 * while an editor is attached should be authorizable without restarting the
 * engine.
 *
 * The three refusals here — unknown server, stdio server, no `auth: "oauth"` —
 * are the same three `arcturn mcp auth` makes, and are made before any network
 * request, so a typo cannot become a discovery attempt against whatever the
 * name happened to resolve to.
 */
export function createMcpAuthBroker(options: McpAuthBrokerFactoryOptions): McpAuthBroker {
  return new McpAuthBroker({
    runFlow: runMcpOAuthFlow,
    ...(options.timeoutMs === undefined ? {} : { timeoutMs: options.timeoutMs }),
    resolveServer: async (name: string) => {
      const [{ loadMcpConfig }, { expandEnvVars }] = await Promise.all([
        import("@arcturn/mcp"),
        import("./mcp-cli.js"),
      ]);
      const files = [...new Set([options.paths.userMcp, options.paths.projectMcp])].filter((file) =>
        existsSync(file),
      );
      ...
      if (server.type !== "http") {
        throw new Error(`"${name}" is a stdio server; OAuth applies to http servers only`);
      }
      if (server.auth !== "oauth") {
        throw new Error(`"${name}" is not configured for OAuth; add "auth": "oauth" to its entry`);
      }
      return {
        serverUrl: expandEnvVars(server.url, options.env ?? process.env),
        storage: new FileMcpOAuthStorage(options.paths.auth, name),
      };
    },
  });
}
