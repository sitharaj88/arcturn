...
```bash
arcturn add sitharaj88/arcturn/examples/complexity-guard
```

That is the GitHub `owner/repo/subdir` shorthand: Arcturn clones the repo, uses
`examples/complexity-guard` as the package root, pins the resolved commit in
...
```bash
arcturn inspect sitharaj88/arcturn/examples/complexity-guard
```
...
```bash
arcturn add ./examples/complexity-guard
...
const warnings = [];
const defs = await loadAgentDefs(["examples/complexity-guard/agents"], warnings, BUILT_IN_TOOL_NAMES);
...
```
=== AGENTS: loadAgentDefs(examples/complexity-guard/agents) ===
...
`arcturn inspect ./examples/complexity-guard` prints the same three lanes, the
