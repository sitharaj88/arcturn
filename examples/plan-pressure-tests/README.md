...
```bash
arcturn add sitharaj88/arcturn/examples/plan-pressure-tests
```

That is the GitHub `owner/repo/subdir` shorthand: Arcturn clones the repo, uses
`examples/plan-pressure-tests` as the package root, pins the resolved commit in
...
```bash
arcturn inspect sitharaj88/arcturn/examples/plan-pressure-tests
```

From a clone, the local-path form works for both — `arcturn add ./examples/plan-pressure-tests`
and `arcturn inspect ./examples/plan-pressure-tests` — which is also the loop for
...
different output table, an extra never — change them. `arcturn add ./examples/plan-pressure-tests`
