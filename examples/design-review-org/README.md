...
```bash
arcturn add sitharaj88/arcturn/examples/design-review-org
```

That is the GitHub `owner/repo/subdir` shorthand: Arcturn clones the repo, uses
`examples/design-review-org` as the package root, pins the resolved commit in
...
```bash
arcturn inspect sitharaj88/arcturn/examples/design-review-org
```
...
```bash
arcturn add ./examples/design-review-org
...
const warnings = [];
const defs = await loadAgentDefs(["examples/design-review-org/agents"], warnings, BUILT_IN_TOOL_NAMES);
...
```
=== AGENTS: loadAgentDefs(examples/design-review-org/agents) ===
...
=== WORKFLOWS: parseWorkflow(examples/design-review-org/workflows) ===
...
`arcturn inspect ./examples/design-review-org` prints the same five lanes, the
