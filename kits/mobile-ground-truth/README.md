...
```bash
arcturn add sitharaj88/arcturn/kits/mobile-ground-truth
```

That is the GitHub `owner/repo/subdir` shorthand: Arcturn clones the repo, uses
`kits/mobile-ground-truth` as the package root, pins the resolved commit in
...
```bash
arcturn inspect sitharaj88/arcturn/kits/mobile-ground-truth
```

From a clone, the local-path form works too — `arcturn add ./kits/mobile-ground-truth` —
...
else — change them. `arcturn add ./kits/mobile-ground-truth` from a clone installs
