...
`examples/starter-skills` as the package root, pins the resolved commit in
...
```bash
arcturn inspect sitharaj88/arcturn/examples/starter-skills
```

From a clone, the local-path form works too — `arcturn add ./examples/starter-skills` —
...
Keep a Changelog — change them. `arcturn add ./examples/starter-skills` from a clone
