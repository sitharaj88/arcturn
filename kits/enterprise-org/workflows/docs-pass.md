...
1. [tier:fast] @docs-writer Inventory the documentation surface for this scope: {{input}} — list every public symbol, command and configuration key in topological dependency order so that everything a symbol depends on is listed before it, mark each one documented, stale or undocumented with the file and line where its documentation lives or should live, and emit ARTIFACT: DOCREC restricted to the inventory. Read the code to decide staleness; never infer it from the age of a file.
