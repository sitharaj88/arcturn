...
```bash
arcturn add sitharaj88/arcturn/examples/design-docs
```

That is the GitHub `owner/repo/subdir` shorthand: Arcturn clones the repo, uses
`examples/design-docs` as the package root, pins the resolved commit in
...
```bash
arcturn inspect sitharaj88/arcturn/examples/design-docs
```

From a clone, the local-path form works for both — `arcturn add ./examples/design-docs`
and `arcturn inspect ./examples/design-docs` — which is also the loop for installing your
...
`arcturn add ./examples/design-docs` from a clone installs your edited copy, and a
